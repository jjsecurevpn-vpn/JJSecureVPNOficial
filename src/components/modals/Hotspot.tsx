    ...
    <Modal onClose={onClose} title={t.hotspot.title} icon={Share2}>
      <div className="flex flex-col h-full">
        ...
        <div className="text-center px-6 pt-6 pb-4">
          <div 
            className={`w-20 h-20 rounded-full mx-auto mb-4 flex items-center justify-center transition-all duration-500 border-2 ${isEnabled ? 'bg-success/10 border-success scale-100' : 'bg-surface-border/30 border-surface-border/70 scale-100'}`}
          >
            {isEnabled ? (
              <Wifi size={32} className="text-success transition-opacity duration-500" />
            ) : (
              <WifiOff size={32} className="text-neutral-text transition-opacity duration-500" />
            )}
          </div>
          <div className="space-y-2">
            <Text 
              ...
              className="font-semibold block transition-colors duration-500"
            >
              {isEnabled ? (t.hotspot?.sharing || "Compartiendo Conexión VPN") : (t.hotspot?.notSharing || "Hotspot Desactivado")}
            </Text>
            <Text variant="body" color="tertiary" className="text-sm max-w-sm mx-auto block transition-opacity duration-500">
              {isEnabled 
        ...
        <div className="flex-1 overflow-auto px-6 space-y-4">
          ...
          <div className="text-center">
            <div className="relative inline-block min-w-48">
              ...
              >
                <span className={`inline-flex items-center gap-2 transition-opacity duration-300 ${loading ? 'opacity-0' : 'opacity-100'}`}>
              ...
              {loading && (
                <div className="absolute inset-0 flex items-center justify-center rounded-lg">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-white">{t.hotspot?.processing || "Procesando..."}</span>
                  </div>
                </div>
              )}
            </div>
          ...
          {/* Estado Activo */}
          {isEnabled && (
            <div className="hotspot-active animate-in fade-in duration-500">
