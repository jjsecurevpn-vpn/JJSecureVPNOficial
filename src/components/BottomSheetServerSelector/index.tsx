...
import { FreeServersInfoModal } from "../modals/FreeServersInfoModal";
import { useAndroidBackButton } from "../../hooks/useAndroidBackButton";

import { useBottomSheetGestures } from "./hooks";
import { ServerInfo, ConnectionButtons, PremiumFeatures, SpeedStats } from "./components";
...
export function BottomSheetServerSelector({
  ...
  const { activeConfig, refreshActiveConfig } = useActiveConfig();
  const [formError, setFormError] = useState<string | null>(null);
  const [isExpanded, setIsExpanded] = useState(false);
  const [showFreeServersInfo, setShowFreeServersInfo] = useState(false);
  ...
  const handleSelectServerFromModal = (serverId: number) => {
    try {
      const success = nativeAPI.config.setActive(serverId);
      if (success) {
        refreshActiveConfig();
      }
    } catch (error) {
  ...
  useAndroidBackButton({
    isActive: showFreeServersInfo,
    onBackPressed: () => setShowFreeServersInfo(false),
    priority: 90,
  });
  ...
  return (
    <>
      ...
      >
        <div className="pointer-events-none absolute -top-[120px] inset-x-0 h-[120px] z-10 bg-gradient-to-b from-transparent via-slate-800/10 to-slate-800" />

        <div
          className="relative h-full shadow-[0_-8px_32px_rgba(0,0,0,0.25),0_-4px_16px_rgba(0,0,0,0.15)] bg-slate-800"
        ...
        >
          ...
          >
            <div
              className={`bg-surface/80 ${classes.borderRadius} ${classes.cardPadding} border border-surface-border shadow-subtle flex flex-col`}
            >
              <div>
                <ServerInfo
                  activeConfig={activeConfig}
                  vpnState={vpn.state}
                  onShowFreeServersInfo={() => setShowFreeServersInfo(true)}
          ...
          >
            <PremiumFeatures onOpenPricingScreen={onOpenPricingScreen} />
          </div>
        </div>
      </div>

      <FreeServersInfoModal
        isOpen={showFreeServersInfo}
        onClose={() => setShowFreeServersInfo(false)}
        onSelectServer={handleSelectServerFromModal}
