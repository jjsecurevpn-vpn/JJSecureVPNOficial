...
// ===== FUNCIONES DEPRECADAS - COMPLETAMENTE REMOVIDAS =====
// Estas funciones fueron removidas. Usa nativeAPI en su lugar:
// - shouldShowInput()    → nativeAPI.auth.shouldShowInput()
// - getActiveConfig()    → nativeAPI.config.getActive()
// - getConnectionState() → nativeAPI.vpn.getState()
// - startConnection()    → nativeAPI.vpn.start()
// - stopConnection()     → nativeAPI.vpn.stop()
// - getDescargaBytes()   → nativeAPI.network.getDownloadBytes()
// - getSubidaBytes()     → nativeAPI.network.getUploadBytes()
