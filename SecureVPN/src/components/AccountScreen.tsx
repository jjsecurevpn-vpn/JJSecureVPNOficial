...
import { dt } from '../utils/nativeApi';
...
export function AccountScreen() {
  ...
  const stats = useMemo(
    () => [
      ...
      { label: 'Descarga', value: formatBytes(dl) },
      { label: 'Subida', value: formatBytes(ul) },
      { label: 'Sesiones activas', value: conexiones.toString() },
    ],
    [statusCopy, pShow, pCls, used, dl, ul, conexiones],
  );
  ...
  const sections = useMemo(
    () => [
      {
        ...
        items: [
          ...
          { label: 'Límite de dispositivos', value: limite },
          { label: 'Conexiones registradas', value: conexiones.toString() },
        ],
      },
      {
    ...
    [name, vence, limite, conexiones, server, mode, topInfo.op, topInfo.ip, creds.user, creds.uuid],
  );
    ...
    <section className="screen account-screen" style={sectionStyle}>
      ...
      <div className="account-grid">
        {sections.map(({ title, icon, items }) => (
          <div key={title} className="account-card">
